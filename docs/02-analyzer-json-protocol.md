...
- 协议 v1 是当前已经实现并由 Java 解析、测试和持久化的兼容边界。
- v0.2 产品方向不会在一次文档改动中破坏协议 v1。
- `quality` 中的分数字段目前不能被宣传为可信的总健康评分；在没有经过验证的公式前可以保持占位值。
- 协议 v2 的设计任务和验收边界见 `docs/tasks/09-report-protocol-v2-design.md`。该任务完成并经双方评审前，本文件中的 v1 仍是唯一已实现协议。
- 协议 v2 当前是 `ACCEPTED`，设计见 [报告协议 v2](protocol/analyzer-report-v2.md)，字段迁移见 [v1 到 v2 迁移方案](protocol/analyzer-report-v2-migration.md)。双方评审已完成且无未解决分歧；在独立实现和默认版本切换任务完成前，本文件中的 v1 仍是唯一已实现且默认输出的协议。
